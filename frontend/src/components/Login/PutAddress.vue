<template>
  <div class="d-flex justify-content-around">
    <select class="form-select form-select-lg m-2" aria-label=".form-select-lg example" @change="selectRegion($event)">
...
export default {
  ...
  methods: {
        selectRegion(event) {
          if (event.target.value === '세종특별자치시') {
            this.userInformation.address = event.target.value
            this.userInformation.region = event.target.value
            this.$emit('getAddress', this.userInformation)
          }
          else {
            this.userInformation.region = event.target.value
            this.userInformation.city = null
            this.userInformation.address = null
            this.$emit('getAddress', this.userInformation)
          }
        },
        selectCity(event) {
            this.userInformation.city = event.target.value
            this.userInformation.address = this.userInformation.region + ',' + this.userInformation.city
            this.$emit('getAddress', this.userInformation)
        },
    },
}
