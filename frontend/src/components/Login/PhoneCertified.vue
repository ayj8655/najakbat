<template>
  <div class="my-3">
    <div class="d-flex justify-content-end">
      <span v-if="!counting && numberConfirmation" class="me-auto"
        >시간이 초과되었습니다</span
      >
      <button
        type="button"
      ...
      >
        <countdown
          v-if="counting"
          :left-time="180000"
          @finish="(countdown) => finish(countdown)"
        >
        ...
        <span v-else-if="!counting && numberConfirmation"
          >인증번호 다시받기</span
        >
      </button>
      <button v-else class="btn btn-success" :disabled="certifiedPhone">
        인증되었습니다
      </button>
    </div>
    <div v-if="counting && !certifiedPhone" class="form-floating mt-3">
      <input
        ...
        v-model="certifiedInput"
      />
      <label for="floatingInput">인증번호 6자리를 입력하세요</label>
      <div class="mt-3 d-flex justify-content-end">
        <span class="me-auto text-danger" v-if="!missCertifiedNumber"
          >인증번호가 다릅니다</span
        >
        <button class="btn btn-success" @click.prevent="checkCertifiedInput">
          인증하기
        </button>
      </div>
    </div>
